...
## usage: texp_cut_and_batch.py [/path/to/input/directory/with/texp/files/] [/path/to/output/directory/] [number of input batches] [number of export batches]
...
texp_cut_id_list = events_texp[events_texp['texp_max (s)'] > 10800]['event_id'].to_list()
...
coverable_filt = np.floor(10.8/events_semicov_sched['texp_sched (ks)'].to_numpy()) <= np.ceil(events_semicov['area(90)'].to_numpy()/10) #True if coverable in perfect world
...
midcov_frac_coverable = len(events_midcov[np.round(10.8/events_midcov['texp_sched (ks)']) > np.round(events_midcov['area(90)']/10)])/len(events_midcov)
